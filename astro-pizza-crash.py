...
class Pizza(Enemy):
    ...
    def __init__(self, game, x, y):
        super(Pizza, self).__init__(image = Pizza.image,
                                    x = x, y = y,
                                    dx = random.choice([1, -1]) * Pizza.SPEED * random.random(),
                                    dy = random.choice([1, -1]) * Pizza.SPEED * random.random())
...
class Game(object):
    ...
    def advance(self):
        ...
        for i in range(self.level):
            ...
            ENEMYS = {"asteroid" : "Asteroid(game = self, x = x, y = y, size = Asteroid.LARGE)",
                      "pizza" : "Pizza(game = self, x = x, y = y)"}
            new_enemy = ENEMYS[(random.randrange(2) - 1)]
